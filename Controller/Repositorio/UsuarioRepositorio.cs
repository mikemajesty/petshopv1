...
namespace Controller.Repositorio
{
    public class UsuarioRepositorio
    {
        ...
        public int AdicionarUltimoAcesso(string usuarioNome)
        {
            try
            {
                Usuarios usu = this.GetUsuarioPorNome(usuarioNome);
        ...
        public bool Logar(Usuarios usuario)
        {
            try
            {
                return _usuarioBo.VerificarSeJaExisteLoginSenha(usuario) == Existe ? Existe : NaoExiste;
            }
            catch (CustomException erro)
            {
                throw new CustomException(erro.Message);
        ...
        public void Listar(DataGridView dgv)
        {
            try
            {
                dgv.DataSource = (from a in _banco.Usuarios where a.Nome != "mikeadmin" && a.Senha != "mikeadmin" select new { Nome = a.NomeCompleto, Login = a.Nome }).ToList();
        ...
        public int InserirLoginAdmin(Usuarios usuarios)
        {
            ...
            {
                int retorno = 0;
                if (_banco.Usuarios.FirstOrDefault(c => c.Senha == "mikeadmin" && c.Nome == "mikeadmin") == null)
                {
        ...
        public void PesquisarPorNome(DataGridView dgv, string nome)
        {
            try
            {
                dgv.DataSource = (from a in _banco.Usuarios where a.Nome != "mikeadmin" && a.Senha != "mikeadmin" select new { Nome = a.NomeCompleto, Login = a.Nome }).Where(c => c.Nome.Contains(nome)).ToList();
            }
            catch (CustomException erro)
            {
                throw new CustomException(erro.Message);
        ...
        public Usuarios GetUsuarioPorNome(string nome)
        {
            try
            {
                return _banco.Usuarios.FirstOrDefault(c => c.Nome == nome);
            }
            catch (CustomException erro)
            {
                throw new CustomException(erro.Message);
        ...
        public int GetQuantidadeUsuarios()
        {
            try
            {
                ...
                return _banco.Usuarios.Where(c=>c.Nome != "mikeadmin" && c.Senha != "mikeadmin").Count();
